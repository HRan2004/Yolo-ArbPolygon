...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, edges=0, classes=None, agnostic=False, multi_label=False,
    ...
    ci = edges*2+5 # index of first class in each box
    nc = prediction.shape[2] - ci  # number of classes
    xc = prediction[..., ci-1] > conf_thres  # candidates
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]):
            l = labels[xi]
            v = torch.zeros((len(l), nc + ci), device=x.device)
            v[:, :4] = l[:, 1:5]  # box
            v[:, 4] = 1.0  # conf
            v[range(len(l)), l[:, 0].long() + ci] = 1.0  # cls
            x = torch.cat((x, v), 0)
        ...
        # Compute conf
        x[:, ci:] *= x[:,ci-1:ci]  # conf = obj_conf * cls_conf
        ...
        if multi_label:
            i, j = (x[:, ci:] > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((box[i], x[i,4:ci-1], x[i, j+ci, None], j[:, None].float()), 1)
        else:  # best class only
            conf, j = x[:, ci:].max(1, keepdim=True)
            x = torch.cat((box, x[:,4:ci-1], conf, j.float()), 1)[conf.view(-1) > conf_thres]

        # Filter by class
        if classes is not None:
            x = x[(x[:, ci-1:ci] == torch.tensor(classes, device=x.device)).any(1)]
        ...
        if not n:  # no boxes
            ...
            x = x[x[:, ci-1].argsort(descending=True)[:max_nms]]  # sort by confidence

        # Batched NMS
        c = x[:, ci:ci+1] * (0 if agnostic else max_wh)  # classes
        boxes, scores = x[:, :4] + c, x[:, ci-1]  # boxes (offset by class), scores
        ...
        if merge and (1 < n < 3E3):  # Merge NMS (boxes merged using weighted mean)
            ...
            x[i, :4] = torch.mm(weights, x[:, :ci-1]).float() / weights.sum(1, keepdim=True)  # merged boxes
            if redundant:
